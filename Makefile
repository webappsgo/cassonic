PROJECTNAME := $(shell basename -s .git "$$(git remote get-url origin 2>/dev/null)" 2>/dev/null || basename "$$(pwd)")
...
test:
	...
	@$(GO_DOCKER) go mod download
	@$(GO_DOCKER) go vet ./...
	@$(GO_DOCKER) go test -buildvcs=false -v -cover -coverprofile=coverage.out ./...
	@echo "Tests complete"
